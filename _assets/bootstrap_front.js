...
(function () {
	...
	if ((b.supported && !b.disabled) || b.forced) {
		...
		for (var i=0; i<scriptTags.length; i++)
			if (scriptTags[i].src && scriptTags[i].src.indexOf('bootstrap_front.js') > 0)
				scriptTag = scriptTags[i];

		var includedFiles = {};
		
		scriptTag.src.split("bootstrap_front.js?").pop().replace(/(\w+)=([^;]*)/g, function ($0, type, files) {
